...
final class Auth extends UserManager {
		...
		if (!$this->isLoggedIn()) {
			...
			if (isset($_COOKIE[$this->rememberCookieName])) {
				...
				if (!empty($parts[0]) && !empty($parts[1])) {
					try {
						$rememberData = $this->pdb->selectRow(
	...
	private function resyncSessionIfNecessary() {
		...
		if ($this->isLoggedIn()) {
			...
			if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= \time()) {
				...
				try {
					$authoritativeData = $this->pdb->selectRow(
	...
	public function convertOldUser($user_id, callable $callback = null) {
    

		$oldUserData = self::getOldUserDataById($user_id);
    if (!$this->userExistsByUserId($user_id)){
      $usermail = explode(';',$oldUserData['email']);
      if (is_array($usermail)){
        $oldUserData['email'] = $usermail[0];
      }
      
      $usr = $this->admin()->createUserWithUniqueUsername($oldUserData['email'], $oldUserData['password'], $oldUserData['username']);
  ...
  public function getAllOldUsers($limit = "1000"){
    try {
			$users = $this->pdb->select('select * from table_logins where id not in (select id from users) order by id asc limit '.$limit);
      return $users;
		}
		catch (Error $e) {
			throw new DatabaseError($e->getMessage());
	...
	public function reconfirmPassword($password) {
		if ($this->isLoggedIn()) {
			...
			try {
				$expectedHash = $this->pdb->selectValue(
	...
	public function logOut() {
		...
		if ($this->isLoggedIn()) {
			...
			unset($_SESSION[self::SESSION_CLASSIFIED]);
		}
	}
	...
	public function confirmEmail($selector, $token) {
		...
		try {
			$confirmationData = $this->pdb->selectRow(
	...
	public function changeEmail($newEmail, callable $callback) {
		if ($this->isLoggedIn()) {
			...
			try {
				$existingUsersWithNewEmail = $this->pdb->selectValue(
	...
	private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback) {
		try {
			$latestAttempt = $this->pdb->selectRow(
	 ...
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
	private function userExistsByUserId($id) {
		try {
			$userData = $this->pdb->selectRow(
				'SELECT id FROM ' . $this->dbTablePrefix . 'users WHERE id = ?',
				[ $id ]
			);
		}
		catch (Error $e) {
	 ...
	 * @return int the status as one of the constants from the {@see Status} class
	 */
	public function getClassifiedSession() {
		if (isset($_SESSION) && isset($_SESSION[self::SESSION_CLASSIFIED])) {
			return $_SESSION[self::SESSION_CLASSIFIED];
		}
		else {
			return null;
		}
	...
	public function admin() {
		return new Administration($this->pdb, $this->dbTablePrefix);
	}
	...
	private function getRememberDirectiveExpiry() {
		...
		if ($this->isLoggedIn()) {
			...
			if (isset($existingSelector)) {
				...
				$existingExpiry = $this->pdb->selectValue(
