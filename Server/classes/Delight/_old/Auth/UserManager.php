...
abstract class UserManager {
	...
	/** @var string session field for the counter that keeps track classifieds data in the current session */
	const SESSION_CLASSIFIED = 'classified_session';
	...
	protected function __construct($databaseConnection, $dbTablePrefix = null) {
		if ($databaseConnection instanceof PdoDatabase) {
			$this->pdb = $databaseConnection;
		}
		elseif ($databaseConnection instanceof PdoDsn) {
			$this->pdb = PdoDatabase::fromDsn($databaseConnection);
		}
		elseif ($databaseConnection instanceof \PDO) {
			$this->pdb = PdoDatabase::fromPdo($databaseConnection, true);
		}
		else {
			$this->pdb = null;
	...
	protected function createUserInternal($requireUniqueUsername, $email, $password, $username = null, callable $callback = null) {
		...
		if ($requireUniqueUsername) {
			...
			if ($username !== null) {
				...
				$occurrencesOfUsername = $this->pdb->selectValue(
		...
		$newUserId = (int) $this->pdb->getLastInsertId();
	...
	protected function onLoginSuccessful($userId, $email, $username, $status, $roles, $forceLogout, $remembered) {
		...
		$_SESSION[self::SESSION_CLASSIFIED] = $this->getDbSession($userId);
	}
  
  protected function getDbSession($userId){
      $custom_id = md5(uniqid(rand(),1));
      $custom_id = substr( $custom_id, 0, 32);
      $this->pdb->delete(
				$this->dbTablePrefix . 'geodesic_sessions',
				[
	...
	/**
	 * Returns the requested old user data for the account with the specified old_id (if any)
	 * Retrieve from geodesic_logins and geodesic_userdata
   *
	 * You must never pass untrusted input to the parameter that takes the column list
	 *
	 * @param string $old_id the old_id to look for
	 * @param array $requestedColumns the columns to request from the user's record
	 * @return array the user data (if an account was found unambiguously)
	 * @throws UnknownUsernameException if no user with the specified old_id has been found
	 * @throws AmbiguousUsernameException if multiple users with the specified old_id have been found
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
	//protected function getOldUserDataById($old_id, array $requestedColumns) {
	protected function getOldUserDataById($old_id) {
		try {
			//$projection = \implode(', ', $requestedColumns);

			$users = $this->pdb->select(
				'SELECT l.*, u.email FROM geodesic_logins l, geodesic_userdata u WHERE l.id = ?  and l.id = u.id LIMIT 2 OFFSET 0',
				[ $old_id ]
			);
		}
		catch (Error $e) {
